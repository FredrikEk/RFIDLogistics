...
void curl_connect2(SendData *send_data, const char *URL) {
    curl_connect5(send_data, URL, NULL, NULL, 0);
}

void curl_connect3(SendData *send_data, const char *URL, const char *post_data) {
    curl_connect5(send_data, URL, post_data, NULL, 0);
}
...
void curl_connect5(SendData *send_data, const char *URL, const char *post_data,
    ...
    if(curl){
        ...
        if (headers) {
            // Create a CURL header list
            struct curl_slist *chunk = NULL;
            // Append the headers to the CURL list
            for (i = 0; i < num_headers; ++i) {
                chunk = curl_slist_append(chunk, (*headers)[i]);
            }
            // Finally set the headers to the request
