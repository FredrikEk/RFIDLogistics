...
char* call_reader()
{
    ...
    */
    ...
    if(fd < 0){
        return strdup("usb not connected");
        perror(MODEMDEVICE); 
        exit(-1);
    }
    ...
    */
    ...
    // Remove empty reads; Reads only containing init byte 10
    if(res == 1 && buf[0] == 10)
        return NULL;

    // The tags we are reading have 12 or 13 bytes
    int start, end, size;
    if(res == 12){
        start = 1;
        end = 11;
        ...
        printf("rfid read data with wrong size: %i\n", res);
        return NULL;
    }
    size = end - start;
